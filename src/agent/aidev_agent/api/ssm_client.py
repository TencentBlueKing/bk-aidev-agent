...
class SSMClient:
    ...
    def get_user_access_token(self, username: str = None, bk_token: str = None) -> str:
        """
        获取用户态access_token

        如果不提供参数，将尝试从request上下文获取用户信息

        Args:
            username: 用户名（如果为空则从request上下文获取）
            bk_token: 用户登录token（如果为空则从request上下文获取）
