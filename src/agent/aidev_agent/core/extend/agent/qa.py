...
class CommonQAStreamingMixIn:
    ...
    def stream_standard_event(self, agent_e, cfg, input_, skip_thought=True, timeout: int = 2):
        ...
        if is_deepseek_r1_series_models(self.llm) or ("deepseek-v3" or "hunyuan-t1" in self.llm.model_name):
            # 用于去除 think 标识位
            max_cache_length = 50
...
class CommonQAAgent(ToolCallingCommonQAAgent):
    ...
    def get_agent_executor(cls, *args, **kwargs):
        ...
        key = (
            ...
            if (is_deepseek_r1_series_models(llm) or ("deepseek-v3" or "hunyuan-t1" in llm.model_name)) and extra_tools
