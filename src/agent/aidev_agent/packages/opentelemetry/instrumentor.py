...
from .otel_service import AgentOTelService
...
class BkAidevAgentInstrumentor(BaseInstrumentor):
    def __init__(self, config: Optional[OTelConfig] = None):
        ...
        self._otel_service: Optional[AgentOTelService] = None

    def start_otel_service(self):
        if self._otel_service is None:
            self._otel_service = AgentOTelService(self._otel_service_config)
    ...
    def _instrument(self, tracer=None, **kwargs):
        ...
        拦截 langchain_core.callbacks.BaseCallbackManager.__init__ 方法,
        自动将 callback_handler 添加到所有 CallbackManager 实例中。
