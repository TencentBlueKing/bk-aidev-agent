...
class BkAidevAgentCallbackHandler(BaseCallbackHandler):
    ...
    def on_bk_agent_start(
        ...
        inputs: Dict[str, Any],
        session_id="unknown",
        call_system_bk_app_code="bkai",
        call_system_ai_type="ai_chat",
        executor="anonymous",
        model_name="unknown",
        ...
        **kwargs: Any
        ...
        try:
            ...
            # Agent 配置信息
            agent_info = get_agent_config_info(executor)
            agent_id = agent_info.get("agent_id") or "unknown"
            agent_code = agent_info.get("agent_code") or "unknown"
            agent_name = agent_info.get("agent_name") or "unknown"
            agent_type = agent_info.get("agent_type") or "agent"
            agent_service_catalogue = agent_info.get("agent_service_catalogue") or "unknown"
            agent_updater = agent_info.get("agent_updater") or "unknown"
            # SDK 版本
            try:
            ...
            attributes = {
                ...
                "agent.info.updater": agent_updater,
                # 会话维度
