...
# 中断回调（Command resume）场景下顶层 span gen_ai.input.messages 的表达文案：
# 恢复驱动时本轮不存在新的用户消息，以固定文案保留「由中断恢复」这一语义
...
class BkAidevAgentCallbackHandler(AsyncCallbackHandler):
    ...
    def _build_chain_input_messages(self, inputs: Any) -> str:
        """构造顶层 chain span 的 gen_ai.input.messages（OTel parts 模型 JSON）。

        三种来源，按优先级：
        1. 中断回调（``Command``）：图由外部中断恢复驱动，本轮没有新的用户消息，
           统一以固定文案表达该语义；
        2. 正常调用：自 ``inputs["messages"]`` 自后向前取第一条 user 消息，只表达本轮
           输入而非全量 history；整段 history 无 user 消息时退化为最后一条；
        3. 上述均不可用：退回构造期快照 ``self._start_inputs``（通常是用户输入文本，
           含未配注入器的裸 handler 场景），保持与旧属性同等的信息量。
        """
        ...
        if isinstance(messages, list) and messages:
            last_user = next(
                (msg for msg in reversed(messages) if _message_type_to_role(msg.type) == "user"),
                messages[-1],
            )
            return json.dumps([_chat_message_to_parts_dict(last_user)], ensure_ascii=False)
        if self._start_inputs is None:
            return ""
        return json.dumps([{"role": "user", "parts": [{"type": "text", "content": str(self._start_inputs)}]}])

    @staticmethod
    def _build_chain_output_messages(outputs: Any) -> str:
        """构造顶层 chain span 的 gen_ai.output.messages（OTel parts 模型 JSON）。

        取 ``outputs["messages"]`` 中自后向前的第一条 AI 消息作为本轮产出。
        取不到（outputs 非 dict、无 messages、无 AI 消息）时返回空串，由调用方跳过写入。
        """
        ...
        for msg in reversed(messages):
            if isinstance(msg, AIMessage) or getattr(msg, "type", None) == "ai":
                return json.dumps([_chat_message_to_parts_dict(msg)])
        return ""
    ...
    async def on_chain_start(
        ...
        **kwargs: Any,
        ...
        if is_top_level:
            # 本轮调用输入以 OTel 官方 parts 模型表达，与 LLM span 上的同名属性语义一致：
            # 中断回调（Command）写固定文案；无可表达输入时不写该键（区别于旧 input.value 的恒写入）
            input_messages = self._build_chain_input_messages(inputs)
            if input_messages:
                _set_content_attributes(
                    self.spans[run_id].span,
                    content_key="gen_ai.input.messages",
                    original_length_key="gen_ai.input.messages_original_length",
                    truncated_key="gen_ai.input.messages_truncated",
    ...
    async def on_chain_end(
        ...
        **kwargs: Any,
        ...
        if is_top_level_workflow:
            ...
            # 本轮产出以 OTel 官方 parts 模型表达（末条 AI 消息），与 LLM span 上的同名属性语义一致；
            # 取不到 AI 消息时不写该键。必须在 _end_span 之前写入，否则 span 已结束不再接收属性
            output_messages = self._build_chain_output_messages(outputs)
            if output_messages:
                _set_content_attributes(
                    span,
                    content_key="gen_ai.output.messages",
                    original_length_key="gen_ai.output.messages_original_length",
                    truncated_key="gen_ai.output.messages_truncated",
                    value=output_messages,
                    max_attribute_length=self.max_output_attribute_length,
                )
