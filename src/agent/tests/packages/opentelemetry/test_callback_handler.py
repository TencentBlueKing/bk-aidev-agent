...
class TestBkAidevAgentCallbackHandler:
    ...
    async def test_top_level_chain_span_carries_input_messages(self, tracer_and_exporter):
        """顶层 chain span 带 gen_ai.input.messages（parts 模型），子链无该键，且均无 input.value"""
        tracer, exporter = tracer_and_exporter
        handler = BkAidevAgentCallbackHandler(tracer=tracer, start_inputs="agent-level")
        chain_id, child_id = uuid4(), uuid4()
        # 回调 inputs 的 messages 为唯一来源：与构造期快照取不同值以锁定来源
        await handler.on_chain_start(
            {"name": "wf"},
        ...
        chain_attrs = handler.spans[chain_id].span.attributes
        child_attrs = handler.spans[child_id].span.attributes
        assert json.loads(chain_attrs["gen_ai.input.messages"]) == [
            {"role": "user", "parts": [{"type": "text", "content": "hello"}]}
        ]
        assert "input.value" not in chain_attrs
        assert "gen_ai.input.messages" not in child_attrs
        assert "input.value" not in child_attrs

    @pytest.mark.parametrize("start_inputs", [None, "agent-level"])
    async def test_top_level_chain_span_input_messages_fallback(self, tracer_and_exporter, start_inputs):
        """inputs 无可用 messages 时的兜底：有快照退化为快照文案，无快照则不写该键"""
        tracer, exporter = tracer_and_exporter
        handler = BkAidevAgentCallbackHandler(tracer=tracer, start_inputs=start_inputs)
        chain_id = uuid4()
        await handler.on_chain_start({"name": "wf"}, {}, run_id=chain_id, parent_run_id=None)
        attrs = handler.spans[chain_id].span.attributes
        assert "input.value" not in attrs
        if start_inputs is None:
            assert "gen_ai.input.messages" not in attrs
        else:
            assert json.loads(attrs["gen_ai.input.messages"]) == [
                {"role": "user", "parts": [{"type": "text", "content": "agent-level"}]}
            ]

    @pytest.mark.parametrize(
        "history, expected",
        [
            # 多轮 history 以 human 结尾：只表达本轮的 user 消息
            (
                [HumanMessage(content="first"), AIMessage(content="a1"), HumanMessage(content="second")],
                "second",
            ),
            # history 非 user 结尾：回溯取最后一条 user 消息
            (
                [
    ...
    async def test_top_level_chain_input_messages_takes_last_user(self, tracer_and_exporter, history, expected):
        """inputs["messages"] 分支只取最后一条 user 消息（无 user 时退化为最后一条）"""
        tracer, exporter = tracer_and_exporter
        handler = BkAidevAgentCallbackHandler(tracer=tracer, start_inputs="agent-level")
        chain_id = uuid4()
        await handler.on_chain_start({"name": "wf"}, {"messages": history}, run_id=chain_id, parent_run_id=None)
        attrs = handler.spans[chain_id].span.attributes
        assert json.loads(attrs["gen_ai.input.messages"]) == [
            {"role": "user", "parts": [{"type": "text", "content": expected}]}
        ]

    async def test_top_level_chain_input_messages_from_command_resume(self, tracer_and_exporter):
        """inputs 为 Command（中断回调）：gen_ai.input.messages 写固定文案「来自中断回调」"""
        tracer, exporter = tracer_and_exporter
        handler = BkAidevAgentCallbackHandler(tracer=tracer, start_inputs="agent-level")
        chain_id = uuid4()
        await handler.on_chain_start({"name": "wf"}, Command(resume="yes"), run_id=chain_id, parent_run_id=None)
        attrs = handler.spans[chain_id].span.attributes
        assert json.loads(attrs["gen_ai.input.messages"]) == [
            {"role": "user", "parts": [{"type": "text", "content": "来自中断回调"}]}
        ]

    async def test_top_level_chain_input_value_removed_but_root_session_input_kept(self, tracer_and_exporter):
        """input.value 已移除；根 span 冻结的 agent.session.input 不受影响（仍为 str(inputs)）"""
        tracer, exporter = tracer_and_exporter
        handler = self._make_root_span_handler(tracer)
        chain_id = uuid4()
        await handler.on_chain_start(
        ...
        assert root_span.attributes["agent.session.input"] == str({"input": "x"})
        assert "input.value" not in chain_span.attributes
