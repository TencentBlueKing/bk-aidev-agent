...
class TestFilesAppendSideEffect:
    ...
    def test_user_image_falls_back_to_read_image_without_vision(self):
        """本轮图片 + 非多模态主模型：降级成路径文本，由模型调 read_image 识别。"""
        chat_prompt = ChatPrompt(
            id="img2",
            role=PromptRole.USER_IMAGE.value,
            content="![图](https://example.com/files/upload_file.jpeg)",
        ...
        assert "read_image" in messages[0].content[0]["text"]
        assert "https://example.com/files/upload_file.jpeg" in messages[0].content[0]["text"]
