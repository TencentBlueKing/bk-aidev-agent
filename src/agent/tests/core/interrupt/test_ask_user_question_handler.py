...
# ---------- _normalize_questions / _first_question_text 脏数据兜底（复刻 2.2.1 a14fe8a7） ----------


# 测试：_normalize_questions 兼容 LLM 输出的各种脏结构
...
# 测试：str 项被包装成完整协议字段（header/multiSelect/options 补齐默认值）
...
# 测试：_first_question_text 缺字段回落链
@pytest.mark.parametrize(
    "questions, expected",
    [
        ([], ""),
...
# 测试：build_payload 接受脏 questions 不再 500
# 回归 —— 修前 questions=["纯字符串问题"] 会触发 TypeError: string indices must be integers。
...
# 测试：build_payload 空 questions 走"需要用户回答"兜底 message
...
# 测试：build_payload 处理 dict 项缺 question 字段时 message 回落 header
