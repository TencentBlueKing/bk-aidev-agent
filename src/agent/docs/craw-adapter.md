...
1. **对话转发（Proxy）**：`CrawCompletionAgent` 实现 SDK `AgentProtocol`（`build`/`execute`/`stop`）。默认 HTTP 走 OpenAI 兼容 `/v1/chat/completions`，翻译 `RUN_STARTED → TEXT_MESSAGE_* → RUN_FINISHED`；OpenClaw 设置 `BKAI_OPENCLAW_TRANSPORT=ws` 时改走网关 `chat.send`，在正文之外翻译 `TOOL_CALL_START / ARGS / END / RESULT`。两种事件都经 `event_handler` 自动落库；非流式返回与原生 `ChatCompletionAgent` 同构。
...
| `BKAI_CRAW_TIMEOUT` | HTTP / WebSocket 超时秒 | 300 |
| `BKAI_OPENCLAW_TRANSPORT` | OpenClaw 流式传输；`ws` 可见 AG-UI 工具事件，`http` 为纯文本回退 | `http` |
